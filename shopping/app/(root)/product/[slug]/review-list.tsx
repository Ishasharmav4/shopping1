...
export default function ReviewList({
  ...
  const reload = async () => {
    try {
      ...
      toast.error('Error in fetching reviews', {
        className: 'destructive-toast', // Custom class for destructive styling
      })
    }
  }
  ...
  // const { toast } = useSonner() // Commenting out the old toast usage
  ...
  const onSubmit: SubmitHandler<CustomerReview> = async (values) => {
    ...
    if (!res.success)
      return toast.error(res.message) // Update to use the new toast library for error messages

    setOpen(false)
    reload()
    toast.success(res.message) // Update to use the new toast library for success messages
